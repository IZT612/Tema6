package parte1.ejercicio7SinHashSet;
...
public class Main {
	...
	static public String mostrarTodos() {
		...
		try(BufferedReader out = new BufferedReader(new FileReader("src\\parte1\\ejercicio7SinHashSet\\agenda.txt"))) {
	...
	static public boolean nuevoContacto(String nombre, int numero) {
		...
		if (!estaLleno() && !nombreExistente(nombre)) {
			...
			try(BufferedWriter in = new BufferedWriter(new FileWriter("src\\\\parte1\\\\ejercicio7SinHashSet\\\\agenda.txt", true))) {
